...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function showResult(data) {
        let cls = 'scan-error', icon = '❌', title = '';

        if (data.success) {
            if (data.action === 'checkin') {
                cls = 'scan-success'; icon = '🟢'; title = 'CHECK-IN';
            } else if (data.action === 'checkout') {
                cls = 'scan-info'; icon = '👋'; title = 'CHECK-OUT';
            } else if (data.duplicate) {
                cls = 'scan-warning'; icon = '⚠️'; title = '';
            }
        }
        ...
        if (data.membership_info && data.action !== 'checkout') {
            ...
            if (data.membership_info.remaining !== undefined &&
                data.membership_info.remaining <= 3 && data.membership_info.remaining > 0) {
                html += '<div class="mt-2"><span class="badge bg-danger">Λίγες προπονήσεις απομένουν!</span></div>';
            }
        }
        html += '</div>';
